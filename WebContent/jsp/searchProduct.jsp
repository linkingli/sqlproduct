...
<table align="center" border="1" width="90%">
		<tr>
			<td><input type="button" onclick="deleteAll()" value="删除选中"></td>
				<form method="post",action="${pageContext.request.contextPath}/findCondition">
					<td colspan="10">查询<input type="text"  value="query">查询<input type="submit"  value="确认"></td>
...
<div align="center">
		...
		<a href="${pageContext.request.contextPath}/search?currentpage=1">首页</a>
		<c:if test="{bean.currentpage > 1}">
		<a href="${pageContext.request.contextPath}/search?currentpage=${bean.currentpage-1}">上一页</a>
		</c:if>
		
		<c:forEach begin="1" var="n" end="${bean.totalpage}">
					<c:if test="${bean.currentpage==n}">
								${n}				
						</c:if>
					<c:if test="${bean.currentpage!=n}">
							<a href="${pageContext.request.contextPath}/search?currentpage=${n}">${n}</a>
					</c:if>
				
		</c:forEach>
		
		<c:if test="${bean.currentpage != bean.totalpage}">
		<a href="${pageContext.request.contextPath}/search?currentpage=${bean.currentpage+1}">下一页</a>
		</c:if>
		
	<a href="${pageContext.request.contextPath}/search?currentpage=${bean.totalpage}">末页</a>
</div>
</body>
